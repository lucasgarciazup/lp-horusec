...
const Team: React.FC = () => {
    ...
    const { avatars: { nodes: avatars } } = useStaticQuery(graphql`
        query {
    ...
    return (
        <Container className="mt-5">
            <Row className="row-cols-2">
                {team.map(people => {
                    const image = avatars.find(elem => elem?.name?.toLowerCase() == people?.name?.toLowerCase());
                    return (
                    <Col key={people.name} className='col-4 col-auto mb-3'>
                        <Row>
